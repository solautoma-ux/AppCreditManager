-- Function for Atomic Payment Registration
-- Prevents partial failures and handle balance updates safely
...
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER -- Runs with privileges of the creator (should be admin/postgres)
AS $$
DECLARE
    v_credito RECORD;
...
BEGIN
    ...
    -- 2. Calculate new balances (Prevent negative values)
    -- GREATEST(0, x) ensures we don't violate check constraints even if input is higher
    ...
    SET 
        ...
        estado = CASE WHEN estado = 'liquidado' THEN 'liquidado' ELSE v_nuevo_estado END, -- Keep liquidado if already was
        updated_at = NOW()
    WHERE id = p_credito_id;
